<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="font-black text-2xl text-white uppercase tracking-tighter italic">Modifier Flux</h2>
        </div>
    </x-slot>

    <div class="py-12 px-4">
        <div class="max-w-3xl mx-auto">
            <form action="{{ route('admin.categories.update', $wasteCategory) }}" method="POST" class="bg-white/[0.02] backdrop-blur-xl border border-white/10 rounded-[3rem] p-10 space-y-8 relative overflow-hidden">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-emerald-500 uppercase tracking-[0.2em] italic ml-2">Designation du Materiau</label>
                        <input type="text" name="name" required value="{{ old('name', $wasteCategory->name) }}" placeholder="ex: Plastique PET"
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" name="points_per_kg" step="0.01" min="0" required value="{{ old('points_per_kg', $wasteCategory->points_per_kg) }}" placeholder="10.00"
                            class="w-full bg-[#020617] border border-white/10 rounded-2xl py-4 px-6 text-white text-sm focus:border-emerald-500 focus:ring-0 transition-all placeholder:text-gray-700">
                        @error('points_per_kg')
                            <p class="text-rose-400 text-xs font-bold ml-2">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" name="co2_saved_per_kg" step="0.01" min="0" required value="{{ old('co2_saved_per_kg', $wasteCategory->co2_saved_per_kg) }}" placeholder="2.50"
                            class="w-full bg-[#020617] border border-white/10 rounded-2xl py-4 px-6 text-white text-sm focus:border-emerald-500 focus:ring-0 transition-all placeholder:text-gray-700">
                ...
                <div class="space-y-2">
                    <label class="text-[10px] font-black text-emerald-500 uppercase tracking-[0.2em] italic ml-2">Specifications Techniques</label>
                    <textarea name="description" rows="4" placeholder="Décrivez les conditions d'acceptation..."
                        class="w-full bg-[#020617] border border-white/10 rounded-3xl py-4 px-6 text-white text-sm focus:border-emerald-500 focus:ring-0 transition-all placeholder:text-gray-700">{{ old('description', $wasteCategory->description) }}</textarea>
                    @error('description')
                        <p class="text-rose-400 text-xs font-bold ml-2">{{ $message }}</p>
                    @enderror
                </div>
